...
namespace LeadMicroservices.Controllers
{
    ...
    {
        [HttpGet("GetLeadContactListByCompanyId")]
        [Authorize]
        public async Task<ActionResult<IEnumerable<LeadContactList>>> GetLeadContactListByCompanyId(int CompanyId)
        {
            EconnectContext _context = new EconnectContext();

            var lead = await _context.LeadContactList
                                    .FromSqlRaw("EXEC GetLeadContactListByCompanyId @CompanyId = {0}", CompanyId)
                                    .ToListAsync();
            return Ok(lead);
        }
